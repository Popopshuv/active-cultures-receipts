...
- All animation goes through GSAP. No Framer Motion, AnimeJS, Motion One, or hand-rolled CSS keyframes for anything beyond `transition-opacity` on hover. Import GSAP from `@/lib/gsap` (which registers `ScrollTrigger` once and refreshes positions after fonts/transitions) — never `import gsap from "gsap"` directly in a component.
- **Titles, headings, eyebrows, and short labels get the per-word reveal.** Wrap them in `<RevealText>`. By default, do NOT wrap long body copy in `<RevealText>` — only do that on full content sections when **explicitly asked**.
- **Other content fades in as a block.** Wrap body copy, lists, cards, images, or any group in `<Reveal>` (`src/components/Reveal.tsx`), a lightweight wrapper over `useReveal` (default preset `fade-up`). Don't leave content static — but don't reach for `<RevealText>` when `<Reveal>` is the right tool.
- For finer non-text control, use `useReveal` from `@/lib/useReveal` directly. Prefer a named preset (`fade`, `fade-up`, `lift`, `mask`, `scale`) over a custom callback. Don't author one-off `gsap.to` calls in components — extend the presets in `useReveal.ts` if a recurring pattern is missing.
...
- No flash on load: reveal targets render their hidden state into the markup (`<Reveal>` does this automatically; `RevealText` uses masks). For an above-the-fold reveal, use `<Reveal>` rather than calling `useReveal` on a bare element, or the element flashes visible for a frame before animating. Don't remove the reduced-motion / `<noscript>` fallbacks in `globals.css` and `layout.tsx` that keep hidden content from getting stuck.
...
- Site-wide metadata (title, description, share-link previews) lives in **one file: `src/lib/siteConfig.ts`**. Edit it there — `app/layout.tsx` and `app/opengraph-image.tsx` both read from it. Don't hardcode title/description in `layout.tsx`.
- When a project's purpose changes, update `siteConfig.ts` first (`name`, `title`, `description`, `url`). This is what makes shared links render correctly on iPhone/Slack/Twitter.
- Don't remove `metadataBase` from `layout.tsx` or delete `app/opengraph-image.tsx` — without them, link previews break on Vercel.
- Per-route OG cards are only for pages that genuinely need a different preview; the site default covers most screens.
